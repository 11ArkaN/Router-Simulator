...
The packet plane is the point of the whole thing. Frames are encoded, serialized onto links with propagation delay and bounded queues, and forwarded for real. It is dual stack. ARP and IPv6 neighbor discovery are learned and aged, duplicate address detection and router advertisement run per interface, and MLD tracks multicast listeners. Connected, static, OSPFv2 and OSPFv3 routes feed per-router RIB selection and versioned FIB generations for both address families, with equal-cost multipath and scoped indirect next hops. IPv4 and IPv6 forwarding cover TTL and hop limit, fragmentation and reassembly, Path MTU discovery, extension headers and ICMP and ICMPv6 errors. A host can send an ICMP Echo request to any entered IPv4 destination instead of being limited to another host already present on the canvas. Host and router pings ride real packets, so failing a link or pulling a line card cascades through everything downstream.
