...
class LinkDirection final {
 public:
  ...
  [[nodiscard]] std::optional<Admission> try_transmit(
      std::uint32_t packet_handle, std::size_t captured_octets,
      Clock::time_point now = Clock::now()) noexcept {
    if (in_flight_.full() || !bits_per_second_) return std::nullopt;
    ...
    const auto mac_octets = std::max<std::uint64_t>(captured_octets + 4U, 64U);
    ...
    if (!in_flight_.try_push({packet_handle, admission.delivered})) return std::nullopt;
    return admission;
  }

  [[nodiscard]] bool pop_delivered(std::uint32_t& packet_handle,
                                   Clock::time_point now = Clock::now()) noexcept {
    ...
    InFlight front;
    if (!in_flight_.try_peek(front) || front.delivered > now) return false;
  ...
  [[nodiscard]] std::optional<Clock::time_point> next_delivery() const noexcept {
    InFlight front;
    if (!in_flight_.try_peek(front)) return std::nullopt;
 ...
 private:
  ...
  [[nodiscard]] std::chrono::nanoseconds nanoseconds_for(
