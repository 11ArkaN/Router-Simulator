...
void cli_tests() {
  ...
  const auto no_routes = router::execute_cli(state, session, "show router route-table", ping);
  ...
  router::execute_cli(state, session, "configure card 1 card-type iom4-e", ping);
  ...
  if (!state.card_provisioned || !state.mda_provisioned) {
    throw std::runtime_error("MD-CLI did not provision fresh hardware");
  }
  const auto candidate = router::execute_cli(state, session, "delete card 1", ping);
  if (state.card_present || !session.candidate_dirty) {
    throw std::runtime_error("MD-CLI changed running state before commit");
  }
  if (candidate.find("*[ex:/]") == std::string::npos) {
    throw std::runtime_error("MD-CLI candidate marker is missing from the prompt");
  }
  ...
  if (compare.find("- card 1") == std::string::npos ||
    ...
    throw std::runtime_error("MD-CLI compare did not represent candidate removals");
  }
  router::execute_cli(state, session, "commit", ping);
  if (state.card_provisioned) throw std::runtime_error("MD-CLI commit did not remove provisioning");

  const auto switched = router::execute_cli(state, session, "//", ping);
  if (switched.find("A:R1#") == std::string::npos) {
    throw std::runtime_error("Session did not switch to the classic CLI prompt");
  }
  router::execute_cli(state, session, "configure card 1 card-type iom4-e", ping);
  router::execute_cli(state, session, "configure card 1 mda 1 mda-type me10-10gb-sfp+", ping);
  if (!state.mda_provisioned || state.mda_present) {
    throw std::runtime_error("classic CLI provisioning changed physical equipment");
  }
  ...
  if (!session.candidate_dirty || !session.candidate_outdated || !state.candidate_mda ||
      stale_prompt.find("!*[ex:/]") == std::string::npos) {
    throw std::runtime_error("classic CLI overwrote or hid a dirty MD candidate");
  }
  ...
  if (stale_commit.find("baseline is out of date") == std::string::npos) {
    throw std::runtime_error("MD-CLI accepted a commit from an outdated baseline");
  }
  router::execute_cli(state, session, "discard", ping);

  router::execute_cli(state, session, "configure system name edge-r1", ping);
  router::execute_cli(state, session, "configure port 1/1/1 admin-state disable", ping);
  router::execute_cli(state, session, "configure port 1/1/1 ethernet mtu 1400", ping);
  router::execute_cli(state, session, "configure port 1/1/1 description \"host-a\"", ping);
  router::execute_cli(state, session,
                      "configure router \"Base\" static-routes route 203.0.113.0/24 next-hop 198.51.100.2",
                      ping);
  router::execute_cli(state, session, "commit", ping);
  if (std::string_view(state.system_name.data()) != "edge-r1" ||
      state.ports[0].admin_enabled || state.ports[0].mtu != 1400 ||
      std::string_view(state.ports[0].description.data()) != "host-a" ||
      !state.static_routes[0].valid) {
    throw std::runtime_error("MD-CLI canonical configuration did not commit atomically");
  }
  ...
  const auto default_ping = router::execute_cli(
      state, session, "ping 198.51.100.2", counted_ping);
  if (requested_count != 5 || default_ping.find("ping-result") == std::string::npos) {
    throw std::runtime_error("SR OS ping default count was not applied");
  }
  router::execute_cli(state, session, "ping 198.51.100.2 count 1", counted_ping);
  if (requested_count != 1) throw std::runtime_error("Explicit ping count was ignored");
  ...
  if (router::complete_cli(state, session, "show router ar") != "show router arp" ||
      router::complete_cli(state, session, "show").find("show port") == std::string::npos) {
    throw std::runtime_error("CLI completion is not backed by executable commands");
  }
}
