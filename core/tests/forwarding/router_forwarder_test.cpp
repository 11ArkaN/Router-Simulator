// RouterForwarder service, IPv6, multicast and relay cases. Each scoped
// fixture owns its mutable forwarding state and uses shared wire-level support.
...
void router_forwarder_tests() {
  ...
  require((forwarder->echo_outcome(49U) & 0xffU) == 1U &&
              ((forwarder->echo_outcome(49U) >> 8U) & 0xffU) ==
                  device_catalog::default_ip_hop_limit &&
              (forwarder->echo_outcome(49U) >> 16U) == 2'000'000U,
  ...
  // The protocol publishes RFC 5286 repair state separately from the normal
  // equal-cost set. Forwarding must keep using the lower-cost primary while
  // its port is operational, then use the repair only after that exact egress
  // fails. Both next-hop MACs were learned from encoded ARP above.
  const std::array protected_route{
  ...
  require(rib.rebuild(with_system, {}, protected_route, 2U) &&
              forwarder->program_fib(rib.compile(4U)),
          "forwarder rejected a FIB generation containing an LFA");
  first_port.icmp_redirects_enabled = false;
  second_port.icmp_redirects_enabled = false;
  require(forwarder->configure_port(first_port) &&
              forwarder->configure_port(second_port),
          "LFA fixture could not disable same-interface redirects");
  const auto protected_frame = packet::icmp_echo(
      host_b, router_b, {10U, 0U, 1U, 2U}, {198U, 18U, 0U, 1U}, false, 53U);
  emitted.clear();
  forwarder->receive(1U, protected_frame, &emitted, collect);
  require(emitted.size() == 1U && emitted.front().port == 0U,
          "healthy primary egress did not precede its LFA");
  first_port.operational = false;
  require(forwarder->configure_port(first_port),
          "LFA fixture could not fail the primary port");
  emitted.clear();
  forwarder->receive(1U, protected_frame, &emitted, collect);
  require(emitted.size() == 1U && emitted.front().port == 1U,
          "failed primary egress did not activate its loop-free alternate");
  first_port.operational = true;
  require(forwarder->configure_port(first_port),
          "LFA fixture could not restore the primary port");
  ...
  require(forwarder6->originate_ipv6_echo(
              ...
              ((forwarder6->ipv6_echo_outcome(38U) >> 8U) & 0xffU) ==
                  device_catalog::default_ip_hop_limit &&
              (forwarder6->ipv6_echo_outcome(38U) >> 16U) == 0U &&
              emitted.empty(),
  ...
  require(raised_pmtu->ipv6_path_mtu.front().mtu == 1'300U &&
              ...
              ((successful_outcome >> 8U) & 0xffU) ==
                  device_catalog::default_ip_hop_limit &&
              (successful_outcome >> 16U) == 1'000'000U &&
