...
void spsc_stress_test() {
  ...
  producer.join();
  if (failed) throw std::runtime_error("SPSC release/acquire ordering lost sequence integrity");
}
...
void device_queue_tests() {
  ...
  std::uint32_t value{};
  if (!queue.try_pop(value) || value != 11 || !queue.try_pop(value) || value != 12) {
  ...
  if (!handle || pool.available() + 1 != available || pool.get(*handle)[0] != 0x5a) {
    throw std::runtime_error("Packet pool handle did not preserve frame ownership");
  }
  pool.release(*handle);
  if (pool.available() != available) throw std::runtime_error("Packet pool leaked a slot");
}
...
void ethernet_timing_tests() {
  ...
  if (!first || !second ||
      std::chrono::duration_cast<std::chrono::nanoseconds>(second->started - first->started)
              .count() != 68 ||
      std::chrono::duration_cast<std::chrono::nanoseconds>(first->delivered - origin)
              .count() != 1000058) {
    throw std::runtime_error("Ethernet serialization or propagation timing is invalid");
  }
  ...
  std::uint32_t handle{};
  if (direction->pop_delivered(handle, first->delivered - std::chrono::nanoseconds(1)) ||
      !direction->pop_delivered(handle, first->delivered) || handle != 1 ||
      direction->pop_delivered(handle, first->delivered)) {
    throw std::runtime_error("Link direction exposed an in-flight frame too early");
  }
  ...
  if (!before_change || !after_change ||
      std::chrono::duration_cast<std::chrono::nanoseconds>(
          before_change->delivered - origin).count() != 158 ||
      std::chrono::duration_cast<std::chrono::nanoseconds>(
          after_change->delivered - origin).count() != 326) {
    throw std::runtime_error("Link propagation update changed an existing deadline");
  }
  ...
  for (std::uint32_t value = 0; value < 2048; ++value) {
    if (!saturated->try_transmit(value, 60, origin)) {
      throw std::runtime_error("Link direction rejected traffic before capacity");
    }
  }
  if (saturated->try_transmit(2048, 60, origin)) {
    throw std::runtime_error("Link direction did not enforce in-flight capacity");
  }
}
...
int main() {
  try {
    ...
    if (initial.find("\"card1Provisioned\":\"absent\"") == std::string::npos ||
      ...
      throw std::runtime_error("Fresh lab invented provisioning or hardware ports");
    }
    ...
    if (invalid_links.rfind("ERROR:", 0) != 0) {
      throw std::runtime_error("Runtime accepted a negative link propagation delay");
    }
    const auto configured_links = runtime->command("project:links|100|250");
    if (configured_links.rfind("ERROR:", 0) == 0) {
      throw std::runtime_error("Runtime rejected an atomic per-link timing update");
    }
    const auto invalid_host = runtime->command(
        "project:hosts|invalid|192.0.2.2/30|192.0.2.1|02:00:00:00:00:0B|198.51.100.2/30|198.51.100.1");
    if (invalid_host.rfind("ERROR:", 0) != 0) {
      throw std::runtime_error("Runtime accepted malformed project endpoint data");
    }
    const auto invalid_identity = runtime->command(
        "project:hosts|01:00:5E:00:00:01|192.0.2.3/30|192.0.2.1|02:00:00:00:00:0B|198.51.100.2/30|198.51.100.1");
    if (invalid_identity.rfind("ERROR:", 0) != 0) {
      throw std::runtime_error("Runtime accepted group MAC or broadcast IPv4 endpoint data");
    }
    // A pair transaction must permit a valid identity swap. Sequential endpoint
    // commands would reject the first half against the still-old second host and
    // leave control and forwarding with different project generations.
    const auto swapped = runtime->command(
        "project:hosts|02:00:00:00:00:0B|192.0.2.2/30|192.0.2.1|02:00:00:00:00:0A|198.51.100.2/30|198.51.100.1");
    if (swapped.rfind("ERROR:", 0) == 0) {
      throw std::runtime_error("Atomic endpoint transaction rejected a valid identity swap");
    }
    runtime->command("project:hosts|02:00:00:00:00:0C|192.0.2.2/30|192.0.2.1|02:00:00:00:00:0B|198.51.100.2/30|198.51.100.1");
    ...
    if (final.find("02:00:00:00:00:0C") == std::string::npos) {
      throw std::runtime_error("Edited host MAC did not reach ARP learning in the runtime");
    }
    const auto checkpoint_view = runtime->export_checkpoint();
    const std::vector<std::uint8_t> checkpoint(checkpoint_view.begin(), checkpoint_view.end());
    runtime->command("link:down:1/1/1");
    if (!runtime->import_checkpoint(checkpoint) ||
        runtime->command("terminal:show router arp").find("02:00:00:00:00:0C") ==
            std::string::npos) {
      throw std::runtime_error("Structural checkpoint did not restore active ARP state");
    }
    const auto capture_status = runtime->command("capture:prepare");
    const auto capture = runtime->prepared_capture();
    if (capture_status.rfind("capture ready: ", 0) != 0 || capture.size() < 48 ||
        capture[0] != 0x0a || capture[1] != 0x0d || capture[2] != 0x0d ||
        capture[3] != 0x0a) {
    ...
    for (std::size_t interface_id = 0;
         interface_id < router::profile::capture_interface_names.size(); ++interface_id) {
      if (read32(block) != 1 || read32(block + 4) < 32 ||
          capture[block + 16] != 2 || capture[block + 17] != 0) {
        throw std::runtime_error("PCAPNG interface description is missing if_name");
      }
    ...
    if (read32(block) != 6 || read32(block + 8) != 0 ||
        ...
        capture[block + 69] != 1) {
      throw std::runtime_error("PCAPNG did not preserve captured ARP bytes or interface ID");
    }
    std::array<std::uint8_t, 4> observed_ttl{};
    std::size_t icmp_count = 0;
    while (block < capture.size()) {
      const auto length = read32(block + 4);
      if (length < 32 || block + length > capture.size() || read32(block + length - 4) != length) {
        throw std::runtime_error("PCAPNG enhanced packet block length is invalid");
      }
    ...
    if (icmp_count != 4 || observed_ttl != std::array<std::uint8_t, 4>{64, 63, 64, 63}) {
      std::ostringstream detail;
      detail << "Forwarding TTL sequence invalid: count=" << icmp_count << " values=";
      for (const auto ttl : observed_ttl) detail << static_cast<unsigned>(ttl) << ',';
    ...
    const auto long_ping = runtime->command("terminal:ping 198.51.100.2 count 100");
    if (long_ping.find("100 packets transmitted, 100 packets received") ==
            std::string::npos ||
        long_ping.find("ERROR: response exceeds") != std::string::npos) {
      throw std::runtime_error("Long CLI output exceeded the response mailbox contract");
    }
    ...
    if (degraded.find("\"prefix\":\"192.0.2.0/30\"") == std::string::npos ||
      ...
      throw std::runtime_error("Interface failure did not withdraw only its connected route");
    }
    ...
    if (dropped.find("\"droppedPackets\":1") == std::string::npos ||
        dropped.find("\"lastDropReason\":\"route-miss\"") == std::string::npos) {
      throw std::runtime_error("Failed forwarding did not publish its drop reason");
    }
    runtime.reset();

    if (!rs_create()) throw std::runtime_error("Wasm C ABI did not create runtime");
