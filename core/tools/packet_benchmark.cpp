...
int main() {
  ...
  // three paths have reached stable optimized Wasm code before the clock starts.
  ...
  const auto warmup_ingress = icmp_echo(source, target, source_ip, target_ip, false, 1);
  ...
  }
  {
    ...
    for (std::uint32_t index = 0; index < warmup_iterations; ++index) {
      ...
      if (!admission || !warmup_link->pop_delivered(delivered, admission->delivered)) return 2;
  ...
  const auto ingress = icmp_echo(source, target, source_ip, target_ip, false, 1);
  const auto forward_started = std::chrono::steady_clock::now();
  for (std::uint32_t index = 0; index < iterations; ++index) {
    const auto egress = route_ipv4(ingress, target, source);
    if (egress) sink += (*egress)[22] + (*egress)[34];
  }
  const auto forward_elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(
  ...
  // are asserted separately, so host scheduler jitter cannot corrupt this metric.
  // Heap ownership mirrors LabNetwork and avoids consuming the small Wasm entry
  // stack with the 2048-entry in-flight ring used by the link owner.
  ...
  for (std::uint32_t index = 0; index < link_iterations; ++index) {
    ...
    std::uint32_t delivered{};
    if (!admission || !link->pop_delivered(delivered, admission->delivered)) return 2;
    link_now = admission->delivered;
    sink += delivered & 1U;
  }
  const auto link_elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(
      std::chrono::steady_clock::now() - link_started);


  const auto encoded_frames = static_cast<double>(iterations) * 2.0;
  std::cout << "{\"encodedFrames\":" << static_cast<std::uint64_t>(encoded_frames)
            << ",\"forwardedFrames\":" << iterations
            << ",\"linkFrames\":" << link_iterations
            << ",\"encodingNsPerFrame\":" << encode_elapsed.count() / encoded_frames
