...
Wasm offsets remain stable while a browser-side typed-array view is rebuilt after a memory epoch change. Packet-path allocation is bounded and resource exhaustion remains observable. Exact ASIC buffer sizes are not claimed where the vendor does not publish them. Such queue values remain documented emulator profiles and require measurements before being described as hardware-accurate.
