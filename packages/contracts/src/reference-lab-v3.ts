...
import { createEmptyProjectV3, createRouterProjectV3, equippedRouterPorts,
  parseLabProjectV3, PROFILE_CATALOG, type HostProjectV3, type LabProjectV3,
  type RouterProjectV3 } from "./lab-project-v3";

function equipEthernetCard(router: RouterProjectV3): void {
...
function port(router: RouterProjectV3, ordinal: number): string {
...
function configureRouter(router: RouterProjectV3,
...
function host(id: string, name: string, macSuffix: string,
  address: string, gateway: string): HostProjectV3 {
...
export function createFourRouterReferenceLabV3(): LabProjectV3 {
  const project = createEmptyProjectV3(new Date("2026-07-16T00:00:00.000Z"));
  ...
  const r1 = createRouterProjectV3("r1", fixedProfile.id, "R1");
  const r2 = createRouterProjectV3("r2", compactModular.id, "R2");
  const r3 = createRouterProjectV3("r3", largestModular.id, "R3");
  const r4 = createRouterProjectV3("r4", compactModular.id, "R4");
  ...
  project.links = [
    { id: "h1-r1", endpoints: [{ nodeId: "h1", portId: "eth0" },
      { nodeId: "r1", portId: port(r1, 0) }], admin: "up", propagationDelayNs: 100 },
    { id: "r1-r2", endpoints: [{ nodeId: "r1", portId: port(r1, 1) },
      { nodeId: "r2", portId: port(r2, 0) }], admin: "up", propagationDelayNs: 100 },
    { id: "r2-h2", endpoints: [{ nodeId: "r2", portId: port(r2, 2) },
      { nodeId: "h2", portId: "eth0" }], admin: "up", propagationDelayNs: 100 },
    { id: "r2-r3", endpoints: [{ nodeId: "r2", portId: port(r2, 1) },
      { nodeId: "r3", portId: port(r3, 0) }], admin: "up", propagationDelayNs: 100 },
    { id: "r3-h3", endpoints: [{ nodeId: "r3", portId: port(r3, 2) },
      { nodeId: "h3", portId: "eth0" }], admin: "up", propagationDelayNs: 100 },
    { id: "r3-r4", endpoints: [{ nodeId: "r3", portId: port(r3, 1) },
      { nodeId: "r4", portId: port(r4, 0) }], admin: "up", propagationDelayNs: 100 },
    { id: "r4-h4", endpoints: [{ nodeId: "r4", portId: port(r4, 2) },
      { nodeId: "h4", portId: "eth0" }], admin: "up", propagationDelayNs: 100 },
    { id: "r1-r4", endpoints: [{ nodeId: "r1", portId: port(r1, 2) },
      { nodeId: "r4", portId: port(r4, 1) }], admin: "up", propagationDelayNs: 100 }
  ];
  ...
  return parseLabProjectV3(project);
}
